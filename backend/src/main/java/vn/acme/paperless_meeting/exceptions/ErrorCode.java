...
public enum ErrorCode implements BaseErrorCode {
        ...
        MEETING_ONLY_DRAFT_ALLOWED(1220, "Hành động này chỉ cho phép khi cuộc họp ở trạng thái NHÁP", HttpStatus.BAD_REQUEST),
        MEETING_ALREADY_CLOSED_OR_CANCELLED(1221, "Cuộc họp đã kết thúc hoặc đã hủy", HttpStatus.BAD_REQUEST),
        MEETING_MUST_BE_SCHEDULED_BEFORE_30_MINUTES(1222, "Cuộc họp phải được lên lịch trước ít nhất 30 phút", HttpStatus.BAD_REQUEST),
        MEETING_LOCATION_OR_TIME_REQUIRED(1223, "Vui lòng nhập địa điểm hoặc thời gian", HttpStatus.BAD_REQUEST),
        MEETING_LOCATION_TIME_CONFLICT(1224, "Địa điểm hoặc thời gian bị trùng lịch với cuộc họp khác", HttpStatus.BAD_REQUEST),
        MEETING_CANNOT_CANCEL_IN_CURRENT_STATUS(1225, "Không thể hủy cuộc họp ở trạng thái hiện tại", HttpStatus.BAD_REQUEST),
        MEETING_CHECKIN_OPEN_TIME_INVALID(1226, "Thời gian bắt đầu điểm danh phải trước giờ họp và tối đa 30 phút trước đó", HttpStatus.BAD_REQUEST),
        MEETING_CHECKIN_CLOSE_TIME_INVALID(1227, "Thời gian kết thúc điểm danh phải sau giờ họp và tối đa 15 phút sau đó", HttpStatus.BAD_REQUEST),
        MEETING_LATE_AFTER_MINUTES_EXCEEDS_CHECKIN_CLOSE(1228, "Thời hạn đăng ký muộn không được vượt quá thời gian kết thúc điểm danh", HttpStatus.BAD_REQUEST),
        ...
        MEETING_PARTICIPANT_NOT_EDITABLE(1231, "Không thể thay đổi người tham gia trong trạng thái cuộc họp hiện tại", HttpStatus.BAD_REQUEST),
        MEETING_PARTICIPANT_MANAGEMENT_FORBIDDEN(1232, "Bạn không có quyền quản lý người tham gia cuộc họp này", HttpStatus.FORBIDDEN),
        ...
        PARTICIPANT_STATUS_INCONSISTENT(1235, "Trạng thái thư mời và trạng thái tham dự không đồng nhất", HttpStatus.BAD_REQUEST),
        ...
        SYSTEM_PERMISSION_PROTECTED(1306, "Không thể sửa mã code hoặc xóa quyền cốt lõi của hệ thống", HttpStatus.FORBIDDEN),
        // PERMISSION
        ...
        AGENDA_TIME_OUT_OF_MEETING(2002, "Thời gian của nội dung cuộc họp phải nằm trong khoảng thời gian cuộc họp", HttpStatus.BAD_REQUEST),
        AGENDA_TIME_NOT_SEQUENTIAL(2003, "Thời gian của các nội dung cuộc họp phải tuần tự không chồng chéo", HttpStatus.BAD_REQUEST),
        AGENDA_PREPARER_NOT_PARTICIPANT(2004, "Người chuẩn bị tài liệu phải là người tham gia cuộc họp", HttpStatus.BAD_REQUEST),
        AGENDA_MODIFICATION_FORBIDDEN(2005, "Chỉ có người tạo cuộc họp hoặc quản trị viên mới có quyền chỉnh sửa thông tin nội dung cuộc họp", HttpStatus.FORBIDDEN),
        AGENDA_SUBMIT_DOCS_FORBIDDEN(2006, "Chỉ có người chuẩn bị tài liệu được gán mới có quyền upload tài liệu", HttpStatus.FORBIDDEN),
        AGENDA_APPROVE_FORBIDDEN(2007, "Chỉ có người tạo cuộc họp mới có quyền phê duyệt tài liệu nội dung cuộc họp", HttpStatus.FORBIDDEN),
        AGENDA_NOT_APPROVED(2008, "Tất cả các nội dung cuộc họp phải được phê duyệt tài liệu trước khi trình duyệt cuộc họp", HttpStatus.BAD_REQUEST),
        ...
        MOTION_ALREADY_VOTED(2011, "Vấn đề biểu quyết đang trong phiên hoặc đã kết thúc, không thể chỉnh sửa hoặc xóa", HttpStatus.BAD_REQUEST),
