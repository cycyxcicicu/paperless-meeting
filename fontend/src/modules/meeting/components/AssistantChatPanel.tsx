...
export const AssistantChatPanel: React.FC<AssistantChatPanelProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      {/* Main Container */}
      <div className="relative bg-[#F8FAFC] rounded-3xl shadow-2xl w-full max-w-2xl h-[85vh] flex flex-col overflow-hidden animate-in fade-in-0 zoom-in-95 duration-250">
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-4 bg-white border-b border-gray-200">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500 font-medium truncate max-w-sm">{meetingTitle}</p>
            </div>
          </div>
          <button
            onClick={onClose}
            ...
            <div className="flex flex-col items-center justify-center h-full text-center gap-4">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500 mt-1">
                  Thông tin họp, tài liệu, biểu quyết, biên bản... chỉ trong phạm vi cuộc họp đang mở.
                </p>
              </div>
          ...
          {messages.map((m, index) => {
            ...
            return (
              <div key={m.id} className={cn('flex', m.role === 'user' ? 'justify-end' : 'justify-start')}>
                <div
                  className={cn(
                    'max-w-[80%] min-w-0 break-words rounded-2xl px-4 py-2.5 text-sm leading-relaxed shadow-sm',
                    m.role === 'user'
                ...
                >
                  ...
                  {isStreamingThisMessage && m.content === '' ? (
                    <span className="flex items-center gap-2 text-xs text-gray-500 font-medium">
                      ...
                      Trợ lý đang tra cứu dữ liệu cuộc họp...
                    </span>
                  ) : (
        ...
        <div className="border-t border-gray-200 bg-white px-4 py-3">
          <div className="flex items-end gap-2">
            <Textarea
              ...
              className="flex-1 max-h-32 rounded-2xl border-gray-200 focus-visible:ring-[#C8102E]/30 focus-visible:border-[#C8102E] text-sm"
              rows={1}
            />
            {sending ? (
              <Button
                type="button"
                variant="primary"
                className="bg-gray-700 hover:bg-gray-800 rounded-full h-10 w-10 p-0 shrink-0"
              ...
              <Button
                type="button"
                variant="primary"
                className="bg-[#C8102E] hover:bg-[#a80d26] rounded-full h-10 w-10 p-0 shrink-0"
                onClick={handleSend}
                disabled={!input.trim()}
              >
