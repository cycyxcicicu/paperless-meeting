...
const NoiDungHopStep: React.FC<NoiDungHopStepProps> = ({
    ...
    const contentsRef = useRef(data.contents);
    useEffect(() => {
        contentsRef.current = data.contents;
    }, [data.contents]);
    ...
    const handleApprove = async (agendaItemId: string) => {
        try {
            ...
            if (res.success) {
                ...
                toast.error(
                    "Thất bại",
                    res.message || "Không thể phê duyệt tài liệu",
                );
            }
        } catch (error: any) {
            ...
            toast.error(
                "Lỗi kết nối",
                error.message || "Đã xảy ra lỗi khi phê duyệt.",
            );
        }
    };
    ...
    const handleReject = async () => {
        ...
        try {
            const res = await meetingApi.rejectDocs(
                rejectModal.agendaId,
                rejectModal.reason.trim(),
            );
            if (res.success) {
                toast.success(
                    "Đã từ chối tài liệu và gửi lại yêu cầu chỉnh sửa",
                );
                const updatedContents = data.contents.map((c) => {
                ...
                toast.error(
                    "Thất bại",
                    res.message || "Không thể từ chối tài liệu",
                );
            }
        } catch (error: any) {
    ...
    const handleOpenPrepModal = (item: NoiDungItem) => {
        const person = participantOptions.find(
            (opt) => opt.value === item.nguoiChuanBi,
        );
        const latestInstruction = item.feedbacks
        ...
        setPrepModal({
            ...
            instructions:
                item.prepInstructions || latestInstruction?.content || "",
    ...
    const handleConfirmSendPrepRequest = async () => {
        ...
        try {
            const res = await meetingApi.sendPrepRequest(
                meetingId,
                prepModal.agendaId,
                {
            ...
            toast.error(
                "Lỗi hệ thống",
                "Đã xảy ra lỗi khi gửi yêu cầu chuẩn bị tài liệu.",
            );
        } finally {
            setPrepModal(null);
        }
    ...
    const handleDrop = (
        ...
        e.preventDefault();
        if (isReadOnly || draggedIndex === null || draggedIndex === targetIndex)
            return;
    ...
    useEffect(() => {
        ...
        const unsubscribers = data.contents.map((item) => {
            ...
            return subscribe(
                `/topic/meeting/${meetingId}/agenda/${item.id}/chat`,
                (newFeedbacks: any) => {
                    console.log(
                        `Real-time chat update for agenda item ${item.id} (coordinator view):`,
                        newFeedbacks,
                    );

                    const updatedContents = contentsRef.current.map((c) => {
                        if (c.id === item.id) {
                            return {
                                ...c,
    ...
    const handleRemoveBieuQuyetIssue = (contentId: string, issueId: string) => {
        ...
        handleUpdateContent(contentId, {
            bieuQuyetIssues: content.bieuQuyetIssues.filter(
                (i) => i.id !== issueId,
            ),
        });
    };
    ...
    const handleRemoveParticipant = (
        ...
        const updatedParticipants = {
            ...
            [type]: content.thanhPhanThamDu[type].filter(
                (item: any) => item.id !== id,
            ),
        };
                ...
                <div className="mb-4 flex items-start gap-3 bg-blue-50/70 border border-blue-200 rounded-xl p-4 text-blue-800 animate-in fade-in duration-200">
                    ...
                    <div className="space-y-1">
                        <span className="text-sm font-semibold block">
                            Thông báo chương trình họp
                        </span>
                        <p className="text-xs text-blue-700/90 leading-relaxed">
                            Bạn đã cung cấp nội dung chương trình họp ở{" "}
                            <strong>Bước 1 (Chi tiết họp)</strong>. Do đó, việc
                            nhập các nội dung chi tiết tại bước này là{" "}
                            <strong>không bắt buộc</strong>. Bạn có thể bỏ qua
                            hoặc chỉ nhập các nội dung chi tiết nếu cần thiết.
                        </p>
                    </div>
                </div>
            )}
                        ...
                        {data.contents.map((content, index) => (
                            <div
                                ...
                                className={cn(
                                    ...
                                    draggedIndex === index &&
                                        "opacity-50 bg-gray-200 border-dashed border border-gray-400 rounded-lg",
                                )}
                    ...
                    <div className="p-6 space-y-6 bg-white">
                        ...
                        <div className="space-y-2">
                            ...
                            {activeContent.nguoiChuanBi &&
                                (!activeContent.status ||
                                    ...
                                    <div className="space-y-1.5 mt-3">
                                        <Label
                                            htmlFor="prepInstructions"
                                            className="text-sm"
                                        >
                                            Nội dung ghi chú / Hướng dẫn chuẩn
                                            bị tài liệu{" "}
                                        ...
                                        <Textarea
                                            id="prepInstructions"
                                            value={
                                                activeContent.prepInstructions ||
                                                ""
                                            }
                                            onChange={(e) =>
                                                handleUpdateContent(
                            ...
                            {!isReadOnly && activeContent.nguoiChuanBi && (
                                <div className="mt-3">
                                    {activeContent.id &&
                                    !activeContent.id.startsWith("content-") ? (
                                        <FeedbackChatSection
                                        ...
                                        <div className="space-y-2">
                                            ...
                                            <p className="text-xs text-amber-600 font-medium bg-amber-50 border border-amber-100 p-2 rounded-xl">
                                                Vui lòng lưu phiên họp trước khi
                                                thực hiện trao đổi/phản hồi.
                                            </p>
                                        </div>
                                    )}
                                </div>
                                        ...
                                        if (newFiles.length > 0) {
                                            ...
                                            try {
                                                ...
                                                for (const file of newFiles) {
                                                    const res =
                                                        await meetingApi.uploadDocument(
                                                            file,
                                                            file.name,
                                                            "OTHER",
                                                        );
                                                    if (
                                                        ...
                                                        toast.error(
                                                            `Tải file ${file.name} thất bại: ${res.message || ""}`,
                                                        );
                                                    }
                                                }
                                                handleUpdateContent(
                        ...
                        {activeContent.status === "PENDING_APPROVAL" &&
                            !isReadOnly && (
                                <div className="flex items-center justify-between gap-4 bg-amber-50/50 border border-amber-200 rounded-xl p-4 animate-in fade-in duration-200">
                                    <div className="space-y-0.5">
                                        ...
                                        <p className="text-[11px] text-amber-700/80">
                                            Vui lòng kiểm tra kỹ các file tài
                                            liệu đính kèm bên trên trước khi
                                            thực hiện phê duyệt.
                                        </p>
                                    </div>
                                    <div className="flex items-center gap-2 shrink-0">
                                        ...
                                        <Button
                                            type="button"
                                            onClick={() =>
                                                handleApprove(activeContent.id)
                                            }
                            ...
                            {activeContent.bieuQuyetIssues.length === 0 ? (
                                <div className="border border-gray-400 rounded-xl p-8 text-center">
                                    <p className="text-sm text-gray-500">
                                        Chưa có vấn đề cần biểu quyết
                                    </p>
                                </div>
                            ) : (
                                <div className="space-y-3">
                                    {activeContent.bieuQuyetIssues.map(
                                        (issue, index) => (
                                            <div
                                                key={issue.id}
                                                className="border border-gray-400 rounded-xl p-4 hover:border-gray-500 transition-colors"
                                            >
                ...
                >
                    <div className="space-y-4 py-2">
                        ...
                        <div className="flex justify-end gap-3 pt-4">
                            <Button
                                variant="outline"
                                className="rounded-xl border-gray-400 hover:border-gray-500"
            ...
            {rejectModal && (
                ...
                >
                    <div className="space-y-4 py-2">
                        <div className="space-y-2">
                            <Label>
                                Lý do từ chối / Hướng dẫn chỉnh sửa{" "}
                                <span className="text-[#C8102E]">*</span>
                            </Label>
                            <Textarea
