...
} from "../components/meeting/AddOpinionForContentModal";
import {
    AddOpinionModal,
    OpinionData,
} from "../components/meeting/AddOpinionModal";
...
import {
    Participant,
    SelectSpeakerModal,
} from "../components/meeting/SelectSpeakerModal";
import { SpeakerActionMenu } from "../components/meeting/SpeakerActionMenu";
import {
    MeetingContent,
    StartContentModal,
} from "../components/meeting/StartContentModal";
...
import {
    Delegate,
    ReadinessCheckModal,
} from "../components/meeting/voting/ReadinessCheckModal";
...
import {
    ...
    VotingResult,
    VotingResultModal,
} from "../components/meeting/voting/VotingResultModal";
...
export default function DienBienPhienHopPage() {
    ...
    const [isStartContentModalOpen, setIsStartContentModalOpen] =
        useState(false);
    const [isApproveContentModalOpen, setIsApproveContentModalOpen] =
        useState(false);
    const [
    ...
    const [currentVotingIssue, setCurrentVotingIssue] =
        useState<VotingIssue | null>(null);
    const [isConfirmBroadcastModalOpen, setIsConfirmBroadcastModalOpen] =
        useState(false);
    const [isReadinessCheckModalOpen, setIsReadinessCheckModalOpen] =
        useState(false);
    ...
    const [isVotingResultModalOpen, setIsVotingResultModalOpen] =
        useState(false);
    ...
    const handleAddSpeakers = (selectedParticipants: Participant[]) => {
        const newSpeakers: Speaker[] = selectedParticipants.map(
            (participant, index) => ({
                id: Date.now() + index,
    ...
    const handleRejectSpeech = (speakerId: number) => {
        ...
        const confirmReject = window.confirm(
            "Bạn có chắc chắn muốn bác bỏ phát biểu này?",
        );
        if (confirmReject) {
