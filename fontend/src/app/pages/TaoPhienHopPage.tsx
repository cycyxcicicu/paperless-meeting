...
import { ChiTietHopStep, ChiTietHopData } from '../components/meeting/ChiTietHopStep';
import {
  ThanhPhanThamDuStep,
  ThanhPhanThamDuData,
} from '../components/meeting/ThanhPhanThamDuStep';
import { NoiDungHopStep, NoiDungHopData } from '../components/meeting/NoiDungHopStep';
import { toast } from '../../lib/toast';

const sidebarItems: SidebarItem[] = [
  { name: 'Danh sách phiên họp', path: '/phien-hop' },
  { name: 'Ký yếu phiên họp', path: '/phien-hop/ky-yeu' },
  { name: 'Tra cứu kết luận', path: '/phien-hop/tra-cuu' },
  { name: 'Quản lý sổ tay', path: '/phien-hop/so-tay' },
];
...
const TaoPhienHopPage = () => {
  ...
  const getMeetingDetail = (meetingId: string) => {
    ...
    return {
      ...
      noiDungChuongTrinh: 'text',
      noiDungChuongTrinhText: 'Thảo luận và triển khai các nhiệm vụ trọng tâm quý II/2026',
      giayMoiFile: null,
      chuTriId: 'user-1',
      donVi: [
      ...
      caNhan: [
        { id: 'user-1', name: 'Ông Trần Văn A', chucVu: 'Bí thư', donVi: 'Thành ủy' },
        { id: 'user-2', name: 'Bà Nguyễn Thị B', chucVu: 'Phó Bí thư', donVi: 'Thành ủy' },
      ],
      khachMoi: [
        { id: 'guest-1', name: 'Ông Lê Văn C', donVi: 'Công ty ABC', email: 'levanc@abc.com' },
      ],
      nhomThanhVien: [
      ...
      contents: [
        {
          id: 'content-1',
          noiDungChiTiet: 'Báo cáo tình hình kinh tế - xã hội quý I/2026',
          thoiGianBatDau: '2026-04-17T08:30',
          thoiGianKetThuc: '2026-04-17T09:30',
          nguoiChuanBi: 'user-3',
          nguoiDuyet: 'user-1',
          taiLieu: [
            { id: 'doc-1', name: 'Báo cáo tình hình Q1.pdf', size: 2400000, url: '#' },
          ],
          bieuQuyetIssues: [
            {
              id: 'issue-1',
  ...
  const loadMeetingData = (meetingId: string) => {
    ...
    try {
      ...
      const step1Data = {
        ...
        noiDungChuongTrinh: meeting.noiDungChuongTrinh,
      ...
      toast.error('Không thể tải dữ liệu', 'Đã xảy ra lỗi khi tải dữ liệu phiên họp. Vui lòng thử lại.');
    } finally {
  ...
  const validateStep1 = (): boolean => {
    ...
    if (
      ...
      new Date(chiTietData.thoiGianBatDau) >= new Date(chiTietData.thoiGianKetThuc)
    ) {
  ...
  const handleSaveDraft = () => {
    ...
    toast.success('Lưu nháp thành công', 'Dữ liệu phiên họp đã được lưu dưới dạng bản nháp');
  };
  ...
  const handleSubmitForApproval = () => {
    ...
    toast.success('Gửi phê duyệt thành công', 'Phiên họp đã được gửi đi. Vui lòng chờ phê duyệt để tiếp tục.');

    // Simulate approval after 2 seconds (in real app, this would come from server/user action)
    setTimeout(() => {
      setApprovalStatus('approved');
      toast.success('Phê duyệt thành công', 'Phiên họp đã được phê duyệt. Bạn có thể gửi phiên họp ngay bây giờ.');
    }, 2000);
  };

  const handleSubmitMeeting = () => {
    if (!isUpdateMode && approvalStatus !== 'approved') {
      toast.warning('Chưa được phê duyệt', 'Vui lòng gửi phê duyệt và chờ được duyệt trước khi gửi phiên họp');
      return;
    }
    ...
    if (isUpdateMode) {
      ...
      toast.success('Cập nhật phiên họp thành công', `Thông tin phiên họp "${chiTietData.tenPhienHop}" đã được cập nhật`);
      navigate(`/phien-hop/${id}`);
    } else {
      // Call create API
      console.log('Create meeting:', payload);
      setApprovalStatus('sent');
      toast.success('Tạo phiên họp thành công', `Đã tạo phiên họp "${chiTietData.tenPhienHop}" thành công`);
      navigate('/phien-hop');
    }
  };
  ...
  return (
    <div className="flex min-h-screen bg-[#F8FAFC]">
      ...
      {/* Main Content */}
      <main className="flex-1 ml-60 py-6">
        {/* Header */}
        <div className="px-8 mb-6">
          <button
            onClick={() => navigate(isUpdateMode ? `/phien-hop/${id}` : '/phien-hop')}
            className="inline-flex items-center gap-2 text-gray-600 hover:text-gray-900 transition-colors mb-3"
          >
            <ArrowLeft className="h-4 w-4" />
            <span className="text-sm font-medium">
              {isUpdateMode ? 'Quay lại chi tiết phiên họp' : 'Quay lại danh sách'}
            </span>
          </button>
          <h1 className="text-2xl font-bold text-gray-900">
            {isUpdateMode ? 'Cập nhật phiên họp' : 'Tạo phiên họp mới'}
          </h1>
        </div>
          ...
          <div className="max-w-[1400px] mx-auto bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
              ...
              {isLoadingData ? (
                <div className="flex items-center justify-center py-12">
                  <div className="text-center">
                    ...
                    <p className="text-gray-600">Đang tải dữ liệu phiên họp...</p>
                  </div>
                </div>
              ) : (
                <>
                  ...
                  {currentStep === 2 && (
                    <ThanhPhanThamDuStep data={thanhPhanData} onChange={setThanhPhanData} />
                  )}
                  ...
                  {currentStep === 3 && (
                    <NoiDungHopStep
                      ...
                      inheritedParticipants={thanhPhanData}
                    />
                  )}
                </>
              )}
            ...
            <div className="border-t border-gray-200 px-8 py-5">
              <WizardFooter
                ...
                onCancel={() => navigate(isUpdateMode ? `/phien-hop/${id}` : '/phien-hop')}
