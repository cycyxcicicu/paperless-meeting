...
import { LocationTable, MeetingLocation } from '../components/meeting-rooms/LocationTable';
...
const sidebarItems: SidebarItem[] = [
  { name: 'Lịch họp', path: '/phong-hop', icon: Calendar },
...
const PhongHopPage = () => {
  ...
  const handleSubmitLocationForm = (locationData: any) => {
    ...
    if (locationFormModal.mode === 'create') {
      toast.success('Thêm địa điểm thành công', `Đã thêm địa điểm "${locationData.name}" vào hệ thống`);
      // TODO: Call API to create location
    } else if (locationFormModal.mode === 'edit') {
      toast.success('Cập nhật địa điểm thành công', `Thông tin địa điểm "${locationData.name}" đã được cập nhật`);
  ...
  const handleConfirmDelete = () => {
    ...
    if (deleteModal.location) {
      toast.success('Xóa địa điểm thành công', `Đã xóa địa điểm "${deleteModal.location.name}" khỏi hệ thống`);
    }
          ...
          <div className="flex items-end justify-between">
            <div>
              ...
              <p className="text-sm text-gray-500">
                Danh sách phòng họp và cơ sở vật chất phục vụ cuộc họp
              </p>
            </div>

            <div className="flex items-center gap-2 text-xs text-gray-500">
              <MapPin className="h-4 w-4 text-[#C8102E]" />
              <span className="font-medium text-gray-700">Cập nhật: 17/04/2026</span>
            </div>
          </div>
        </div>
                ...
                >
                  ...
                  <div className="px-6 py-4 border-t border-gray-100 flex items-center justify-between bg-gray-50/30">
                    <p className="text-sm text-gray-500 font-medium">
                      ...
                      -{' '}
                      <span className="text-gray-900 font-semibold">
                        {Math.min(currentPage * pageSize, filteredLocations.length)}
                      </span>{' '}
                      trong tổng số{' '}
